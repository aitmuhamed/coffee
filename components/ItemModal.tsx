...
const ItemModal: React.FC<ItemModalProps> = ({ item, onClose, onAddToCart }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 transition-opacity duration-300 ${isClosing ? 'opacity-0' : 'opacity-100'}`}>
      ...
      <div className={`relative ${theme === 'dark' ? 'bg-[#1a1210]' : 'bg-white'} w-full max-w-5xl rounded-[3rem] overflow-hidden shadow-2xl transition-all duration-500 transform ${isClosing ? 'scale-95 translate-y-8 opacity-0' : 'scale-100 translate-y-0 opacity-100'}`}>
        <button 
          onClick={handleClose}
          className={`absolute top-8 right-8 z-30 ${theme === 'dark' ? 'bg-white/10 text-white hover:bg-white hover:text-black' : 'bg-white/90 text-coffee-dark hover:bg-coffee-dark hover:text-white'} backdrop-blur-md p-3 rounded-full transition-all shadow-lg active:scale-90`}
        >
        ...
        <div className="flex flex-col lg:flex-row h-full max-h-[90vh] lg:h-[700px]">
          ...
          <div className="lg:w-1/2 p-8 md:p-14 overflow-y-auto no-scrollbar flex flex-col">
            <div className="flex-1">
              <div className="mb-8">
                <h2 className={`text-4xl md:text-5xl font-serif font-bold ${theme === 'dark' ? 'text-white' : 'text-coffee-dark'} mb-4 leading-tight`}>
                  {translated.name}
                </h2>
                      ...
                      <button
                        ...
                        className={`flex-1 flex flex-col items-center py-3 px-4 rounded-2xl border transition-all ${
                          selectedSize?.label === size.label 
                            ? (theme === 'dark' ? 'bg-white text-black border-white shadow-lg' : 'bg-coffee-dark text-white border-coffee-dark shadow-lg shadow-coffee-dark/10')
                            : (theme === 'dark' ? 'bg-white/5 text-gray-400 border-white/5 hover:border-white/20' : 'bg-white text-gray-400 border-gray-100 hover:border-gray-300')
                        }`}
                      >
              ...
              <div className="space-y-10 mb-12">
                <div>
                  ...
                  <p className={`${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'} text-lg font-light leading-relaxed`}>
                    {translated.desc}
                  </p>
                </div>
                
                <div className={`grid grid-cols-3 gap-8 py-8 border-y ${theme === 'dark' ? 'border-white/5' : 'border-gray-100'}`}>
                  <div>
                    <h4 className="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-2">Tasting Notes</h4>
                    <p className={`text-sm ${theme === 'dark' ? 'text-gray-200' : 'text-coffee-dark'} font-medium leading-snug`}>{stats.notes}</p>
                  </div>
                  <div>
                    <h4 className="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-2">Brew Stats</h4>
                    <p className={`text-sm ${theme === 'dark' ? 'text-gray-200' : 'text-coffee-dark'} font-medium leading-snug`}>{stats.method}</p>
                  </div>
                  <div>
                    <h4 className="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-2">Intensity</h4>
                    <p className={`text-sm ${theme === 'dark' ? 'text-gray-200' : 'text-coffee-dark'} font-medium leading-snug`}>{stats.intensity}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex flex-col">
                     ...
                     <div className={`flex items-center space-x-6 ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-gray-50 border-gray-100'} px-5 py-3 rounded-2xl border`}>
                        <button 
                        ...
                        >-</button>
                        <span className={`text-lg font-bold w-6 text-center ${theme === 'dark' ? 'text-white' : 'text-coffee-dark'}`}>{quantity}</span>
                        <button 
                  ...
                  <div className="text-right">
                    ...
                    <PriceDisplay usdPrice={currentPrice * quantity} currency={currency} lang={lang} className={`text-2xl font-bold ${theme === 'dark' ? 'text-white' : 'text-coffee-dark'}`} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-4 pt-6 mt-auto">
              <button 
                ...
                className={`flex-1 ${theme === 'dark' ? 'bg-white text-black' : 'bg-coffee-dark text-white'} py-5 rounded-2xl font-bold hover:bg-coffee-light hover:text-white transition-all shadow-2xl uppercase text-[11px] tracking-[0.2em] flex items-center justify-center space-x-3 active:scale-[0.98]`}
              >
            ...
            <p className="mt-8 text-center text-[9px] text-gray-500 uppercase tracking-[0.3em] font-bold">
