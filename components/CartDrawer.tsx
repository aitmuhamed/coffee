    ...
    <div className="fixed inset-0 z-[60] flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-500 ease-out">
        <div className="p-6 border-b border-gray-100 flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-serif font-bold text-coffee-dark">{t.title}</h2>
              ...
              <div key={`${item.id}-${item.selectedSize?.label || 'default'}`} className="flex gap-4 group">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <PriceDisplay usdPrice={item.price * item.quantity} currency={currency} lang={lang} className="font-bold text-coffee-dark text-sm" />
                  </div>
                </div>
              </div>
            ))
        ...
        {items.length > 0 && (
          <div className="p-6 border-t border-gray-100 bg-gray-50/50">
            <div className="flex justify-between mb-2 text-sm">
              ...
              <PriceDisplay usdPrice={totalUSD} currency={currency} lang={lang} />
            </div>
            <div className="flex justify-between mb-6 text-sm">
              <span className="text-gray-500">{t.fee}</span>
              <PriceDisplay usdPrice={processingFeeUSD} currency={currency} lang={lang} />
            </div>
            <div className="flex justify-between mb-8">
              <span className="text-xl font-serif font-bold text-coffee-dark">{t.total}</span>
              <PriceDisplay usdPrice={totalUSD + processingFeeUSD} currency={currency} lang={lang} className="text-2xl font-bold text-coffee-dark" />
            </div>
            <button className="w-full bg-coffee-dark text-white py-5 rounded-2xl font-bold hover:bg-opacity-95 shadow-xl uppercase text-[10px] tracking-[0.2em]">
              {t.checkout}
            </button>
          </div>
        )}
